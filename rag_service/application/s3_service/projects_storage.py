...
class ProjectsStorageService:
    """S3-хранилище, привязанное к бакету файлов проектов.

    Базовый набор методов — как у остальных бакетов. Разные форматы файлов
    и разграничение доступа для проектов добавятся сюда отдельно.
    """

    def __init__(self, store: S3StorageRepository) -> None:
        self._store = store
        self._bucket = settings.minio_buckets[StorageDomain.PROJECTS]
