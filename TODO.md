...
- [ ] 💡 Идея от пользователя (2026-08-05), не начато: саммари только на уровне подтем верхнего
      порядка, не на каждом вложенном пункте отдельно. Сейчас `summarize_document_chapters_task`
      (`workers/task.py`) идёт по ВСЕМ строкам `document_chapters` подряд, включая вложенные
      (например и "1", и "1.1", и "1.2" — каждая получает свой отдельный вызов `_summarize_chapter`).
      Предложение: если у главы есть подглавы (по `chapter_number` — "1.1"/"1.2" при живой "1"; сам
      `chapter_number` сейчас плоская строка, `Mapped[str]`, никакого explicit parent/depth в схеме
      нет, парсить/группировать пришлось бы по числу точек в номере), то одно саммари делается сразу
      на весь блок "1" (кратко захватывая суть 1.1/1.2 внутри одного текста), а для 1.1/1.2 отдельно
      саммари не генерится — экономит LLM-вызовы (сейчас 1 документ = 45+ вызовов, см. выше) и убирает
      дублирование, когда саммари подглавы почти дословно повторяет то же, что уже попало в саммари
      родителя. Не проработано: как определять "лист" вложенности (когда 1.1 сама имеет 1.1.1 — брать
      её текст целиком или тоже сворачивать на 1.1?), как это соотносится с уже сохранённым
      `document_chapters.summary` на per-chapter уровне (сейчас 1:1 с `chapter_id`, при группировке
      нужно решить — писать групповое саммари в родителя и оставлять `summary=NULL` у детей, или
      дублировать одно и то же значение во все строки группы)
...
- Первый (и пока единственный) админ: логин `admin`, пароль `admin` (заведён вручную через `/auth/register` + `is_superuser=true` напрямую в БД — самостоятельная регистрация не даёт роль admin, только следующих админов может назначать уже существующий админ через вкладку «Пользователи» в админ-панели). Пароль поменяли вручную после первого заведения (был `admin1234`), 2026-08-13 подтверждён живьём. Учётка dev-стенда, для прод-окружения пароль сменить.
