...
- вернуть 1-3 подзадачи `search_docs` с разными формулировками (обычный документный вопрос),
...
Хвост списка обрезается по `gateway.max_subtasks` (12 в `ai_config.toml`).

Обёртки try/except намеренно нет — сбой уходит в общий `except` в `agent_routers.py`,
без тихого фолбэка (см. докстринг `plan_node`).
...
`retrieval_data` строится через `formatters.py::merge_search_docs_results()` —
...
- `rerank_query` — **не** `state.query` напрямую, а первая подзадача плана, у которой в
  `args` есть ключ `"query"` (не обязательно `subtasks[0]`, см. ниже почему), если план есть.
  ...
  **Обновлено 2026-08-05 (B5):** раньше бралось буквально `subtasks[0]` — с появлением
  `get_appendix` как второго тула (его `args` — `{"document_code": ...}`, без ключа
  `"query"`) план вида `[get_appendix, search_docs]` тихо откатывал бы `rerank_query`
  обратно на сырой `state.query`, воспроизводя тот же баг заново. Теперь — явный поиск
  первого подходящего элемента по наличию ключа, а не позиционное обращение.
  ⚠️ Если `reflect` предложил несколько `new_queries`, для повторного rerank всё равно
  берётся только **первая** из них — остальные использовались для поиска, но не участвуют
  в сравнении при реранке.
...
`formatters.py::format_retrieval_item_for_prompt`)
...
3. `need_more` + есть `new_queries` → строит **новый** `PlanOutput` из этих запросов и
   кладёт в `state.plan` — **полностью затирая** план первого круга (см. §6.1, известный
   открытый вопрос про потерю контекста первого круга).
...
2. **Code-gate для приложений** (добавлено 2026-08-05): если `state.appendix_context`
   ...
   **Обновлено 2026-08-05 (B6, частичное смягчение):** регекс раньше проверял только
   ...
   чанки) — `mentions_appendix = _APPENDIX_MENTION_RE.search(state.query) or
   _APPENDIX_MENTION_RE.search(context_str)`. Частичное смягчение, не полный фикс: если
   ни вопрос, ни найденные чанки текстуально не содержат "приложени" (а `reflect` тоже не
   вызвался, §6.3), сигнал по-прежнему не сработает.
...
2. Полный сбой **без единого** токена — одноразовый fallback на нестримящий `generate()`
   ...
   `stream=false`). Если и fallback падает — финальная нота `_Не удалось получить ответ: {exc}_`.
...
| `subtask_results` / `retrieval_data` | `execute_subtasks_node` | **перезаписывается целиком** при повторном вызове — результаты первого круга поиска теряются, не мёржатся (см. §6.1) |
...
| `resolved_docs`, `document_passports`, `expanded_queries` (после старта) | нигде (legacy-ноды не подключены) | остаются дефолтными/пустыми весь прогон |
...
Тебе доступны два инструмента (обновлено 2026-08-05, был только search_docs):
- "search_docs" — гибридный поиск по базе технической документации (dense + BM25). args: {"query": "текст запроса", "doc_filter": null}.
...
Правила:
1. Если вопрос требует информации из базы документации — верни 1-3 подзадачи search_docs с разными
   формулировками (раскрой местоимения из истории, добавь техническую формулировку и синонимы, если вопрос
   расплывчатый). Обычно достаточно одной точной формулировки — не плоди подзадачи без необходимости.
...
2. Если фрагменты есть, но не по теме... — verdict "need_more", new_queries: 1-5 новых
   формулировок с учётом того, что уже найдено...
...
|---|---|---|---|
| `search_docs` | read | ✅ реально работает | оборачивает `RetrievalService.retrieve` |
...
- **Сбой сборки НЕ кэшируется** как пустой реестр — следующий вызов попробует собрать заново
  (в отличие от ML-роутера, который сдаётся навсегда после первого сбоя — здесь смысла в этом
  нет, `rag_service` мог просто ещё не подняться при старте `llm_service`).
...
- **⚠️ Инвалидации нет.** Если документ добавлен/удалён/переиндексирован в `rag_service` **после**
  того, как `llm_service` уже построил реестр — `llm_service` этого не узнает до перезапуска
  процесса. Тот же паттерн, что `AbbreviationExpander` в `rag_service`. Веб-хук-инвалидация
  между сервисами обсуждалась и была осознанно отклонена (см. §6.8) — сегодня это открытый,
  принятый компромисс, не забытая задача.

---

## 6. Ключевые расхождения между кодом и ожидаемым поведением

### 6.1 Потеря контекста первого круга `reflect` (открытый вопрос, `ISSUES.md` A5)

Когда `reflect` просит `need_more`, `execute_subtasks_node` на повторном вызове строит
`retrieval_data` **с нуля** только из подзадач нового плана — чанки первого (неудачного)
круга поиска теряются безвозвратно, не мёржатся. `extract_sources_node` в конце видит
только результаты последнего круга. Не факт, что это баг (возможно осознанное поведение
"не разбавлять сильный второй круг слабым первым"), но задокументировано как открытый
вопрос — решение сознательно отложено.
...
### 6.8 Устаревание реестра документов — известное, принятое ограничение (не забытая задача)

Обсуждалось отдельно от аудита: если документ добавлен в `rag_service` уже после того, как
`llm_service` построил свой реестр (§5.1), `llm_service` узнает об этом только после
перезапуска процесса — `get_appendix`/`list_documents` будут работать по устаревшим данным
(старый список файлов, отсутствие новых). Рассматривался вариант с веб-хуком
(`rag_service` → `POST /llm/internal/documents-changed` → `llm_service` сбрасывает кэш),
инфраструктура для него частично существует в других сервисах
(`INTERNAL_WEBHOOK_TOKEN`/`hmac.compare_digest`, см. `backend/api/notes_routes.py`) — но
реализация была начата и затем явно отменена ("стой не надо веб хук"). В коде от этой попытки
ничего не осталось. Открытый вопрос "чем тогда закрывать staleness" (TTL? периодический
polling? ничем, просто документировать?) обсуждался, но решение не принято — сегодняшнее
поведение: реестр живёт весь процесс без обновления, точка.
...
| `retrieve_multi_node` | Поиск по всем переформулировкам сразу | Заменено `execute_subtasks_node` + `search_docs`-подзадачами |
