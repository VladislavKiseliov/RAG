...
**Обновлено 2026-08-06:** вся диспетчеризация системы висит на одном JSON-ответе дешёвой
модели (`route` — не отдельная ML-классификация, целиком производная от «пустой ли план»)
— доля `smalltalk`/`domain_rag` теперь считается Prometheus-counter'ом
`llm_plan_route_total{route=...}` (`planning_nodes.py::PLAN_ROUTE_COUNT`): если планировщик
начнёт систематически решать «поиск не нужен» на документных вопросах, это будет видно
метрикой, а не жалобой пользователя.
...
(а не заменяют его) — дедуп по `parent_id`. На повторном заходе (`reflect` → `need_more` →
сюда снова) агент больше не теряет то, что нашёл на предыдущем круге под другой
формулировкой — объединённый пул целиком уходит в `rerank` (§2.4), который единообразно
пересчитает score для всех элементов сразу против актуальной формулировки. `extract_sources_node`
(§2.11) в конце теперь тоже видит находки обоих кругов, не только последнего.
**Уточнено 2026-08-06 (тот же день):** при совпадении `parent_id` побеждает **свежий**
элемент безусловно, без сравнения `score` — у элемента из `state.retrieval_data` на
повторном круге score уже калиброванный (0..1 кросс-энкодера из прошлого прохода
`rerank_node`), а у свежего результата — сырой fusion-скор из `rag_service` (другая
шкала/диапазон); сравнивать их напрямую бессмысленно. Не проблема для итогового ответа —
`rerank_node` сразу следом пересчитает score для всего объединённого пула единообразно —
но раньше код молча сравнивал несопоставимые числа для выбора, какая версия дубля
выживает.
...
- `rerank_query` — **не** `state.query` напрямую, а формулировки из плана, если план есть.
  ...
  **Обновлено 2026-08-06 (трижды в тот же день):** во-первых, `search_docs.args` сменил
  форму — раньше это был один ключ `"query": str` на подзадачу (несколько формулировок =
  несколько отдельных подзадач), теперь один ключ `"queries": list[str]` на подзадачу
  (несколько формулировок — один список внутри одной подзадачи, см. §5 про батчинг).
  Во-вторых (промежуточный шаг в тот же день, уже не актуален): пробовали склеивать ВСЕ
  формулировки через пробел в один `rerank_query`. В-третьих (финальная версия): склейка
  отменена — `bge-reranker-v2-m3` обучен на парах "один запрос — один пассаж", склейка
  нескольких формулировок в одну строку — данные вне распределения обучения, разбавляет
  сигнал так же, как разбавлял бы целый `parent_chunk` вместо лучшего child (та же причина,
  что и в первом пункте выше). Теперь `rerank_query` — **последний** элемент `"queries"` со
  всех `search_docs`-подзадач плана (если reflect переформулировал — `state.plan` заменён
  целиком на новый, значит "последний" естественно означает "актуальный после
  переформулировки", не нужно ничего специально отслеживать). `get_appendix`-подзадачи (без
  ключа `"queries"`) по-прежнему пропускаются тем же фильтром.
- **Срез после реранка (добавлено 2026-08-06):** до этой правки `rerank_node` только
  пересортировывал — весь пул (включая слабый хвост) уходил дальше в `build_prompt_node`, а
  после накопления `retrieval_data` между кругами `reflect` (§2.2) пул мог удвоиться (до 12
  parent-чанков вместо 6). `gateway.rerank_final_k` (`ai_config.toml`, старт — 6) режет
  `reordered` до среза перед возвратом — режется здесь, не в `build_prompt_node`, потому что
  `extract_sources_node` (§2.11) тоже читает `state.retrieval_data`: источники на фронте
  должны соответствовать тому, что реально видела LLM, не всему найденному пулу.
...
| `retrieval_data` | `execute_subtasks_node` (мёржит с предыдущим значением, обновлено 2026-08-06), `rerank_node` (пересортировка/новый score + срез до `rerank_final_k`) | **накапливается** между кругами — дедуп по `parent_id`, свежая версия побеждает безусловно (см. §6.1, было "перезаписывается целиком", закрыто) |
...
3. Проверь перед ответом: КОНТЕКСТ содержит ПРЯМОЙ ответ, или только общие/смежные
   сведения (область применения, термины, ссылка на другой документ)? Если только
   смежные — первая строка честно признаёт, что прямого ответа нет, и только потом —
   то смежное, что реально есть. Не заполняй пробел своими знаниями (обновлено
   2026-08-06 по итогам §6.9 — раньше правило было мягче и не сработало на живом
   инциденте с ГИС).
...
5а. (добавлено 2026-08-06) Каждый термин/аббревиатура/название оборудования в ответе
    ОБЯЗАН(-а) буквально быть в КОНТЕКСТЕ — не вводи то, чего там нет, даже если это
    известно из общих знаний. Прямая мера против того же инцидента (§6.9) — модель
    подставила «ЛЭС», которого не было ни в одном найденном чанке.
...
### 6.9 «Обманутый порог» — уверенный сфабрикованный ответ на слабом-но-высокооценённом контексте (ЧАСТИЧНО ЗАКРЫТО, 2026-08-06)
...
Обсуждённый план из пяти пунктов, по убыванию отдачи:
1. **✅ Сделано 2026-08-06.** Ужесточено правило 3/добавлено правило 5а `system_prompt_rag`
   (§4.3) — явный чек-лист «термин из ответа ОБЯЗАН быть в КОНТЕКСТЕ буквально» + «проверь:
   это прямой ответ или только общая тема». Единственный пункт из пяти, который закрывает
   уже случившийся инцидент напрямую, не гипотетический риск — сделан первым.
2. **❌ Не сделано.** Boilerplate-сигнал: детектировать заголовки-разделы («область
   применения», «термины и определения» и т.п.) регэкспом по `headers.title` — не исключать,
   использовать как сигнал слабости в гейте.
3. **❌ Не сделано.** Композитный гейт в `decide_after_rerank`: не только `top_score` одного
   чанка, а «высокий скор И ≥N substantive-чанков выше порога» — самый рискованный пункт,
   трогает единственный гейт для 100% `domain_rag`-ответов.
4. **❌ Не сделано.** Grounding-check ПОСЛЕ генерации — только в лог/метрику, не блокирующий
   (переиспользовать код `citation_validity`/`number_grounding` из `EVAL_PLAN.md`).
5. **❌ Не сделано.** Этот кейс — материал для eval-датасета (категория paraphrase/negative).

**Дополнительно сделано в тот же день, не из исходного списка:** срез `rerank_final_k` после
`rerank_node` (§2.4) — без него накопление `retrieval_data` между кругами `reflect` (§2.2,
тоже сегодняшняя правка) удваивало объём слабого контекста, доезжающего до `generate_node` —
прямо кормило тот же механизм «компенсации своими знаниями». Не решает случай, когда
содержательного раздела в корпусе просто нет (тогда единственно верный исход — честный
`no_data`, за что теперь частично отвечает п.1, полностью — п.4, ещё не реализован).
