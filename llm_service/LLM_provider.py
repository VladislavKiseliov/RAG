...
def _build_answer_messages(current_query: str, data_prompt: FinalPromptData) -> list[dict]:
    """Общая сборка messages для чат-ответа (RAG/chat route) - system + история диалога
    отдельными role-сообщениями (не текстом, см. CURRENT_TURN_TEMPLATE) + последнее
    user-сообщение с summary/контекстом/вопросом."""
